...
function updateSubmitButtonState() {
  const hasErrors = Object.values(formStatus).some(status => status !== 0);
  submitButton.disabled = hasErrors;
  submitButton.classList.toggle('disabled-button', hasErrors);
}
...
function validateEmail(email) {
  ...
  // Listen on focus and provide instructions to fill the forms
  inputFields.forEach(inputField => {
      ...
      inputField.addEventListener("input", () => {
          ...
          if (inputField.id === "inputEmail") {
              ...
              status = validateName(value, inputField.name === 'fname' ? 'first' : 'last');
          }
